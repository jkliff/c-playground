...
#ifdef __debug
		printf ("%s & %s %d %s: %s\n", octet2bin (nibble >> shift) , octet2bin (mask), mask, octet2bin(CLASS_MAP [i]), octet2bin (((nibble >> shift) & mask)));
#endif
...
//					(short)host->h_addr_list[i][0], (short)host->h_addr_list[i][1],
//					(short)host->h_addr_list[i][2], (short)host->h_addr_list[i][3]);
			int j;
			for (j = 0; j < 4; j++) {
